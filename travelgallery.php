...
//        $sql = "SELECT * FROM gallery ORDER BY idGallery DESC";
        ...
        if($page != 1) {
            $paging .= '<li class="page page_start"><a href="./travelgallery.php?page=1">처음</a></li>';
        }

        //첫 섹션이 아니라면 이전 버튼을 생성
        if($currentSection != 1) {
            $paging .= '<li class="page page_prev"><a href="./travelgallery.php?page=' . $prevPage . '">이전</a></li>';
        }
        ...
        for($i = $firstPage; $i <= $lastPage; $i++) {
            if($i == $page) {
                ...
                $paging .= '<li class="page"><a href="./travelgallery.php?page=' . $i . '">' . $i . '</a></li>';
            }
        }
        ...
        if($currentSection != $allSection) {
            $paging .= '<li class="page page_next"><a href="./travelgallery.php?page=' . $nextPage . '">다음</a></li>';
        }

        //마지막 페이지가 아니라면 끝 버튼을 생성
        if($page != $allPage) {
            $paging .= '<li class="page page_end"><a href="./travelgallery.php?page=' . $allPage . '">끝</a></li>';
        }
        ...
        $sql = 'select * from gallery order by idGallery desc' . $sqlLimit; //원하는 개수만큼 가져온다. (0번째부터 20번째까지
        $result = mysqli_query($conn,$sql);
